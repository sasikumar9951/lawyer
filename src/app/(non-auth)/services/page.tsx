  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="w-full h-[500px] lg:h-[600px]">
        <C1 title="Our Services" breadcrumbLabel="Services" />
      </div>

      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {categoriesWithServices.map(
            (category) =>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 items-stretch">
                    {category.services.map((service) => {
                      ...
                      const content =
                        (service.contentJson as ServiceContent | null) || null;
                      const deliverables = content?.blocks?.find(
                        (b) => (b as any).type === "deliverables"
                      ) as { type: string; items?: string[] } | undefined;
                      const items =
                      ...
                      return (
                        <div
                          key={service.id}
                          className="h-full border rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col"
                        >
          ...
          {categoriesWithServices.filter((c) => c.services.length > 0)
            .length === 0 && (
            <div className="bg-white rounded-xl shadow-xl p-6 mt-8 text-center text-gray-500 py-10">
